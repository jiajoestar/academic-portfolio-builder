...
const MembershipOfCouncil = ({ onSaved, existingData, hideButtons = false, mode = 'draft', externalSubmitRef, readOnly = false }) => {
    ...
    const onSubmit = async (data, status = mode) => {
        ...
        try {
            ...
            console.error(err)
            toast.error(sttaus === 'published' ? 'Error publishing activity' : 'Error saving draft')
        }
    }
    ...
    useEffect(() => {
        if (existingData) {
            reset({
                ...
                councilName: existingData.details?.councilName || '',
        ...
        <>
            ...
            <form className='form-container' onSubmit={handleSubmit((data) => onSubmit(data, 'draft'))}>
                <fieldset disabled={readOnly} style={{ border: 'none', padding: 0, margin: 0 }}>
                    <h3>Membership of council</h3>
                    <p>Log all things related to membership of council here.</p>
                    ...
                    <div className='form-group'>
                        ...
                        <input placeholder='Board name' {...register('councilName')} />
...
export default MembershipOfCouncil